...
public interface PersistentLoginMapper {
    ...
    @Insert("INSERT INTO tb_persistent_logins (username, series, token, last_used) VALUES (#{username}, #{series}, #{token}, #{lastUsed})")
    ...
    @Update("UPDATE tb_persistent_logins SET token = #{token}, last_used = #{lastUsed} WHERE series = #{series}")
    void updateToken(@Param("series") String series, @Param("token") String token, @Param("lastUsed") java.sql.Timestamp lastUsed);

    @Select("SELECT username, series, token, last_used FROM tb_persistent_logins WHERE series = #{series}")
    PersistentLogin getTokenForSeries(@Param("series") String series);

    @Delete("DELETE FROM tb_persistent_logins WHERE username = #{username}")
